...
let damian = new Person({ name: 'Damian', age: 34, likes:['Pad see ew', 'sticky rice'] })

// create multiple document with Model.create(arr)
const people = [{name: "Vader", age:100},{name:"Bolo", age:70},{name:"Tiger", age:1000}]
...
Person.find({name:"Damian"}).then( (data) => console.log("Damian found: "+  data))
// FindOne
Person.findOne({name:"Damian"}).then( (data) => console.log("Only one true Damian found: "+  data))

// find by id - 5d9b597c5d876414f40c3a9e
Person.findById('5d9b597c5d876414f40c3a9e').then(data => console.log("Found By Id : " + data))
...
Person.findOneAndUpdate({name: 'Damian'}, {$set:{age:20}}, {new: true}, (err,doc) => {
